            ...
            <div class="row product-single">
                <div class="col-md-6">
                    ...
                    <div class="owl-carousel img-carousel">
                        <div class="item">
                            <a class="btn btn-theme btn-theme-transparent btn-zoom" @desktop data-gal="prettyPhoto" href="{{ asset(env('LARGE').$product->image) }}" @enddesktop><i class="fa fa-plus"></i></a>
                            <a @desktop data-gal="prettyPhoto" href="{{ asset(env('LARGE').$product->image) }}" @enddesktop>
                                <img class="img-responsive" src="{{ asset(env('LARGE').$product->image) }}" alt="{{ $product->name_ar . $product->name_en . $product->description_ar . $product->description_en }}" />
                            </a>
                        </div>
                        @if(!$product->gallery->images->isEmpty())
                        @foreach($product->gallery->images as $img)
                        <div class="item">
                            <a class="btn btn-theme btn-theme-transparent btn-zoom" @desktop data-gal="prettyPhoto" href="{{ asset(env('LARGE').$img->path) }}" @enddesktop>
                                <i class="fa fa-plus"></i>
                            </a>
                            <a @desktop data-gal="prettyPhoto" href="{{ asset(env('LARGE').$img->path) }}" @enddesktop>
                                <img class="img-responsive" src="{{ asset(env('LARGE').$img->path) }}" alt="{{ $product->name_ar . $product->name_en . $product->description_ar . $product->description_en . $img->caption_ar . $img->caption_en}}" />
                            </a>
                        </div>
                        @endforeach
                        @endif
                    ...
                    <div class="row product-thumbnails">
                        <div class="col-xs-2 col-sm-2 col-md-3">
                            <a @desktop data-gal="prettyPhoto" href="{{ asset(env('LARGE').$product->image) }}" @enddesktop>
                                <img class="img-responsive" src="{{ asset(env('LARGE').$product->image) }}" alt="" {{ $product->name }} />
                            </a>
                        </div>
                        @foreach($product->gallery->images as $img)
                        <div class="col-xs-2 col-sm-2 col-md-3">
                ...
                <div class="col-md-6">
                    <div class="back-to-category">
                        ...
                        <div class="{{ app()->isLocale('ar') ? 'pull-left' : 'pull-right' }}">
                            <a class="btn btn-theme btn-theme-transparent btn-previous" href="{{ route('frontend.product.show',$products->first()->id) }}"><i class="fa fa-angle-{{ app()->isLocale('ar') ? 'right' : 'left' }}"></i></a>
                            <a class="btn btn-theme btn-theme-transparent btn-next" href="{{ route('frontend.product.show', $products->last()->id) }}"><i class="fa fa-angle-{{ app()->isLocale('ar') ? 'left' : 'right' }}"></i></a>
                        </div>
                        @endif
                    </div>
                    {{-- <h2 class="product-title">{{ $product->name }}</h2> --}}
                    <h2 class="product-title">{{ trans('general.item_code') }} : {{ $product->sku }}</h2>
                    ...
                    <table>
                        ...
                        <tr>
                            <td>
                                <div class="product-price"><span style="font-size: 14px;">{{ $product->isOnSale ? $product->sale_price : $product->price }} {{ trans('general.kwd') }}</span>
                                </div>
                            </td>
                            <td>
                                @if($product->isOnSale)
                    ...
                    <div class="countdown-wrapper">
                        <input type="hidden" name="counter" value="{{ $product->end_sale }}" id="dealCountValue0">
                        <div id="dealCountdown0" class="defaultCountdown clearfix"></div>
                    </div>
                    @endif
                    ...
                    <hr class="page-divider" />
                    <form class="row variable" method="post" class="cart" action="{{ route('frontend.cart.add') }}">
                        @csrf
                        ...
                        <div class="col-sm-6">
                            <div class="form-group selectpicker-wrapper">
                                ...
                                <select id="color" name="color_id" class="selectpicker input-price" data-live-search="true" data-width="100%" data-toggle="tooltip" title="{{ trans('select_color') }}">
                        ...
                        <div class="col-sm-6">
                            <div class="form-group selectpicker-wrapper">
                                ...
                                <select name="size_id" id="size" class="col-sm-6 size-menu input-price" data-live-search="true" data-width="100%" data-toggle="tooltip" title="{{ trans('select_size') }}">
                        ...
                        <div class="col-lg-12">
                            <div class="buttons">
                                <div class="quantity">
                                    ...
                                    <input class="form-control qty" type="text" name="qty" value="1" title="{{ trans('general.quantity') }}" id="qty">
    ...
    <div class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal" style="padding-left: 15px; padding-right: 15px;"><span aria-hidden="true">&times;</span><span class="sr-only"> {{ trans('general.close') }}</span></button>
                <h4 class="modal-title" id="myModalLabel">{{ trans('general.size_charts') }}</h4>
            </div>
            <div class="modal-body" style="text-align: center;">
                <img src="{{ !is_null($product->size_chart_image) ? asset(env('LARGE').$product->size_chart_image) : asset(env('LARGE').$settings->size_chart) }}" id="imagepreview" style="width: 400px; height: 264px;">
            </div>
