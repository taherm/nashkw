                ...
                <aside class="col-md-3 sidebar visible-lg visible-md visible-sm" id="sidebar">
                    ...
                    {{-- <div class="widget widget-colors">
                        <div class="widget-content">
                            <a class="btn btn-theme-sm" href="{{ route('frontend.product.search') }}">{{ trans('general.remove') }}</a>
            </div>
        </div> --}}
            ...
            <div class="widget-content">
                <ul>
                    ...
                    <li style="border:{{ request()->has('color_id') && request('color_id') == $color->id ? '3px solid darkblue' : null}} ">
                        <a href="{!! request()->fullUrlWithQuery(['color_id' => $color->id]) !!}"><span style="background-color: {{ $color->code }}"></span></a>
                    </li>
                    @endforeach
                </ul>
            </div>
    ...
    <div class="row products grid">
        ...
        <div class="col-md-4 col-sm-6">
            <div class="thumbnail no-border no-padding">
                <div class="media">
                    <a class="media-link" @desktop data-gal="prettyPhoto" href="{{ asset(env('LARGE').$element->image) }}" @elsedesktop href="{{ route('frontend.product.show',$element->id) }}" @enddesktop>
                        <img src="{{ asset(env('THUMBNAIL').$element->image) }}" alt="{{ $element->name_ar . $element->name_en . $element->description_ar . $element->description_en }}" />
                ...
                <div class="caption text-center">
                    <h4 class="caption-title"><a href="{{ route('frontend.product.show.name', ['name' => str_replace(' ', '-',$element->name), 'id' => $element->id]) }}">{{ str_limit($element->name,25,'') }}</a>
                    </h4>
                    <div class="price">
                        @if($element->isOnSale)
                    ...
                    <div class="buttons">
                        <a class="btn btn-theme btn-theme-transparent btn-wish-list" href="{{ route('frontend.favorite.add', $element->id) }}"><i class="fa fa-heart"></i></a>
                        <a class="btn btn-theme btn-theme-transparent btn-icon-left" href="{{ route('frontend.product.show.name', ['name' => str_replace(' ', '-',$element->name), 'id' => $element->id]) }}"><i class="fa fa-shopping-cart"></i>{{ trans('general.view_product_details') }}
                        </a>
